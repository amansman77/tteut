...
export default async function Home() {
  ...
  const [wordRows, recentRows, demandRows] = await Promise.all([
    ...
    env.DB.prepare(
      "SELECT normalized_word as word, search_count as searchCount FROM tt_search_demands WHERE fulfilled = 0 ORDER BY search_count DESC, last_seen_at DESC LIMIT 5"
    ).all<{ word: string; searchCount: number }>(),
  ]);
